...
class SearchWorker(QThread):
    ...
    def __init__(self, prompt: str, memory: SemanticMemory, mode: str = "search"):
        super().__init__()
        self.prompt = prompt
        self.memory = memory
        self.mode = mode
    ...
    def _structure_scraped_data_batch(self, user_query: str, raw_scraped_results: List[str]) -> Tuple[str, List[str]]:
        ...
        successfully_structured_content = []
            ...
            for attempt in range(2): # Retry loop
                try:
                    ...
                    if match:
                        ...
                        successfully_structured_content.append(result_str)
                        self.log_message.emit(f"Source {i+1} successfully structured on attempt {attempt+1}.", "INFO")
                        break # Exit retry loop on success
                    else:
                        self.log_message.emit(f"Source {i+1} response missing tags on attempt {attempt+1}/2.", "WARN")

                except Exception as e:
                    self.log_message.emit(f"AbstractionAgent failed for source {i+1} on attempt {attempt+1}/2: {e}.", "ERROR")
            
            else: # This 'else' belongs to the 'for attempt' loop, executed if 'break' is not hit
                self.log_message.emit(f"Source {i+1} failed abstraction after 2 attempts. Skipping.", "ERROR")
        ...
        return final_combined_summary, successfully_structured_content
    ...
    def _validate_scraped_content_batch(self, content_query_pairs: List[Tuple[str, str]]) -> Tuple[List[str], List[str]]:
        ...
        for i, (content, specific_query) in enumerate(content_query_pairs):
            ...
            response = None
            is_ambiguous = False
            for attempt in range(3): # Total 3 attempts
                try:
                    with ThreadPoolExecutor(max_workers=1) as executor:
                        future = executor.submit(ollama.chat, model=config.VALIDATOR_MODEL, messages=validator_messages, stream=False)
                        response = future.result(timeout=config.OLLAMA_VALIDATION_TIMEOUT)
                    
                    if response:
                        validator_output_lower = response['message']['content'].strip().lower()
                        if '<pass>' in validator_output_lower or '<fail>' in validator_output_lower:
                            self.log_message.emit(f"Validator gave a clear response for source {i+1} on attempt {attempt+1}", "INFO")
                            is_ambiguous = False
                            break # Success, exit retry loop
                        else:
                            is_ambiguous = True
                            self.log_message.emit(f"Validator response AMBIGUOUS on attempt {attempt+1}/3 for '{source_title}'. Retrying...", "WARN")
                
                except TimeoutError:
                    self.log_message.emit(f"Validator TIMED OUT on attempt {attempt+1}/3 for source '{source_title}'. Retrying...", "WARN")
                except Exception as e:
                    self.log_message.emit(f"Validator call FAILED on attempt {attempt+1}/3 for '{source_title}': {e}. Retrying...", "ERROR")

            if response and not is_ambiguous:
                ...
                reason = "Validator agent failed after 3 attempts (timeout, error, or ambiguous responses)."
                failure_reasons.append(f"{source_title}: {reason}")
                self.log_message.emit(f"Source {i+1} FAILED validation for '{source_title}' after 3 attempts.", "ERROR")
    ...
    def run(self):
        try:
            ...
            # --- Get conversation history, common to both modes ---
            relevant_history = self.memory.retrieve_relevant_messages(self.prompt, top_k=3, last_n=4) # Increased last_n for better chat context
            sanitized_history = []
            for msg in relevant_history:
                if msg['role'] == 'assistant':
                    clean_content = re.sub(r'<think>.*?</think>', '', msg['content'], flags=re.DOTALL)
                    clean_content = re.sub(r'<sources>.*?</sources>', '', clean_content, flags=re.DOTALL).strip()
                    sanitized_history.append({'role': 'assistant', 'content': clean_content})
                else:
                    sanitized_history.append(msg)
            ...
            if self.mode == "chat":
                self._log_step("Direct Chat Mode: Bypassing search.")
                self.progress.emit("Responding with existing knowledge...")
                self._narrate_step("The user has requested a direct response without a web search. I will answer from my internal knowledge base.")

                synthesis_system_message = self.synthesis_messages[0]
                current_user_message = {'role': 'user', 'content': self.prompt}
                messages_for_chat = [synthesis_system_message] + sanitized_history + [current_user_message]
                
                final_response = self.get_ollama_response(messages=messages_for_chat)

            elif self.mode == "search":
                self._log_step("Search Mode: Initiating RAG pipeline.")
                ...
                if search_requests:
                    ...
                    if not scraped_content:
                        ...
                        num_scraped = len(scraped_content)
                        num_passed = len(passed_content)
                        is_validation_successful = (num_passed >= 2) or (num_scraped <= 2 and num_passed >= 1)

                        if is_validation_successful:
                            self._log_step(f"Validation successful ({num_passed}/{num_scraped} passed). Proceeding to synthesis.")
                            self._narrate_step(f"Validation complete. {len(passed_content)} source(s) look promising, so I'll extract the key details now.")
                            structured_data, successful_content = self._structure_scraped_data_batch(self.prompt, passed_content)
                            
                            if not successful_content:
                                self.log_message.emit("All validated sources failed abstraction. Responding with available knowledge.", "WARN")
                                prompt_for_no_search = "The web search found sources, but I could not process them correctly. Please answer the user's last question using only your existing knowledge."
                                messages_for_fallback = [synthesis_system_message] + sanitized_history + [current_user_message, {'role': 'user', 'content': prompt_for_no_search}]
                                final_response = self.get_ollama_response(messages=messages_for_fallback)
                            else:
                                ...
                                passed_sources = self._filter_sources_by_passed_content(sources_from_search, successful_content)
                                ...
                                if additional_query:
                                    ...
                                    if additional_scraped_content:
                                        ...
                                        if passed_additional_content:
                                            ...
                                            combined_raw_data = successful_content + passed_additional_content
                                            all_sources_combined = sources_from_search + sources_from_add_search
                                            
                                            final_structured_data, final_successful_content = self._structure_scraped_data_batch(self.prompt, combined_raw_data)
                                            self.progress.emit("Synthesizing final response with all data...")
                                            
                                            final_passed_sources = self._filter_sources_by_passed_content(all_sources_combined, final_successful_content)
                                            sources_used_for_synthesis = final_passed_sources # Replace, don't extend
                                            ...
                                            final_response = synthesis_response_1 # Fallback to first response
                                    else:
                            ...
                            self.log_message.emit(f"Initial validation failed ({num_passed}/{num_scraped} passed). Reasons: {combined_failure_reasons}", "WARN")
                            ...
                            if refined_plan_queries:
                                for i, refined_search in enumerate(refined_plan_queries):
                                    ...
                                    if refined_content:
                                        ...
                                        num_refined_scraped = len(refined_content)
                                        num_refined_passed = len(passed_refined_content)
                                        is_refined_validation_successful = (num_refined_passed >= 2) or (num_refined_scraped <= 2 and num_refined_passed >= 1)

                                        if is_refined_validation_successful:
                                            self.log_message.emit(f"Validator: Refined search content PASSED ({num_refined_passed}/{num_refined_scraped}).", "INFO")
                                            
                                            refined_structured_data, successful_refined_content = self._structure_scraped_data_batch(self.prompt, passed_refined_content)
                                            
                                            if successful_refined_content:
                                                self.progress.emit("Synthesizing refined response...")
                                                
                                                passed_refined_sources = self._filter_sources_by_passed_content(sources_from_refined, successful_refined_content)
                                                ...
                                                break 
                                                 ...
                                                 self.log_message.emit(f"Refined attempt {i+1} FAILED abstraction.", "WARN")
                                        else:
                                            self.log_message.emit(f"Validator: Refined attempt {i+1} also FAILED validation ({num_refined_passed}/{num_refined_scraped}).", "WARN")
                                    else:
                                         self.log_message.emit(f"Refined attempt {i+1} yielded no content.", "WARN")
                            
                            if not refined_content_found:
                                self.log_message.emit("All primary and refined search attempts failed to yield sufficient content.", "ERROR")
                ...
                else: # This block handles when IntentAgent decides no search is needed.
                    self.progress.emit("Responding with existing knowledge...")
    ...
    def scrape_with_enhanced_extraction(self, url: str) -> Tuple[str, bool, int, Dict]:
        ...
        content_length = len(main_content) # rejects anyhting under 300
        if content_length < 300: return f"<result url='{url}' title='{title}' date='{date}' error='Content below quality threshold'></result>", False, 0, {}
        if content_length > 4000: main_content = main_content[:4000] + "..." # sets max level of characters to scrape per site
    ...
    def get_ollama_response(self, messages: list) -> str:
        ...
        for attempt in range(max_retries + 1):
            self.log_message.emit(f"Compiling payload for model (Attempt {attempt + 1}/{max_retries + 1})...", "INFO")
            try:
                with ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(ollama.chat, model=model_name, messages=current_messages, stream=False)
                    response = future.result(timeout=config.OLLAMA_DEFAULT_TIMEOUT)
                
                response_content = response['message']['content']
                
                if '<think>' in response_content and '</think>' in response_content:
                    self.log_message.emit(f"Agent response received", "INFO")
                ...
                self.log_message.emit(f"{model_name} request TIMED OUT on attempt {attempt + 1}. Retrying if possible.", "ERROR")
            except Exception as e:
                self.log_message.emit(f"{model_name} request FAILED on attempt {attempt + 1}: {e}", "ERROR")

            if attempt == max_retries:
                raise Exception(f"{model_name} request failed after multiple retries.")
