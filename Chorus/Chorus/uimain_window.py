...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.current_mode = "search" # 'search' or 'chat'
    ...
    def init_ui(self):
        ...
        # --- Mode Toggle and Status Layout ---
        mode_status_layout = QHBoxLayout(); mode_status_layout.setContentsMargins(0, 0, 0, 5)
        self.mode_toggle_button = QPushButton("🌐 Search Mode"); self.mode_toggle_button.setObjectName("modeToggleButton"); self.mode_toggle_button.setCheckable(True); self.mode_toggle_button.setChecked(False); self.mode_toggle_button.clicked.connect(self.toggle_response_mode)
        ...
        mode_status_layout.addWidget(self.mode_toggle_button); mode_status_layout.addWidget(self.status_label); mode_status_layout.addWidget(self.timer_label); mode_status_layout.addStretch()

        # --- Input Layout ---
        input_layout = QHBoxLayout()
        self.input_field = QTextEdit(); self.input_field.setObjectName("inputField"); self.input_field.setPlaceholderText("Enter your query... (Search is ON)"); self.input_field.setFixedHeight(50)
        ...
        # --- Bottom Bar (Disclaimer and Size Grip) ---
        bottom_bar_layout = QHBoxLayout()
        bottom_bar_layout.setContentsMargins(0, 5, 0, 0) # Add top margin for spacing
        
        disclaimer_label = QLabel("Please double check vital details. We strive for reliability but no system is perfect.")
        disclaimer_label.setObjectName("disclaimerLabel")
        
        self.size_grip = QSizeGrip(footer_frame)
        
        bottom_bar_layout.addStretch() # Pushes content from the left
        bottom_bar_layout.addWidget(disclaimer_label)
        bottom_bar_layout.addStretch() # Pushes content from the right
        bottom_bar_layout.addWidget(self.size_grip, 0, Qt.AlignBottom | Qt.AlignRight)
        
        footer_layout.addLayout(mode_status_layout)
        footer_layout.addLayout(input_layout)
        footer_layout.addLayout(bottom_bar_layout) 
    ...
    def get_stylesheet(self):
        ...
        return f"""
            ...
            #modeToggleButton {{ font-size: 12px; color: #CCCCCC; border: 1px solid #007ACC; background-color: #2D2D2D; padding: 4px 10px; border-radius: 4px; }}
            #modeToggleButton:checked {{ color: white; border: 1px solid #6A9955; background-color: #333; }}
            #statusLabel {{ color: #888888; font-size: 12px; padding-left: 10px; }}
            #timerLabel {{ color: #007ACC; font-size: 12px; font-weight: bold; margin-left: 10px; }}
            #disclaimerLabel {{ color: #777777; font-size: 11px; font-style: italic; }}
            QSizeGrip {{
    ...
    def toggle_response_mode(self):
        if self.mode_toggle_button.isChecked():
            ...
            self.mode_toggle_button.setText("💬 Chat Mode")
            self.input_field.setPlaceholderText("Enter your query... (Search is OFF)")
            self.update_log("Switched to Chat Mode (direct LLM response).", "INFO")
        else:
            self.current_mode = "search"
            self.mode_toggle_button.setText("🌐 Search Mode")
            self.input_field.setPlaceholderText("Enter your query... (Search is ON)")
            self.update_log("Switched to Search Mode (web search enabled).", "INFO")
    ...
    def start_new_chat_session(self):
        ...
        # Reset mode to default search mode for new chats
        if self.mode_toggle_button.isChecked():
            self.mode_toggle_button.setChecked(False)
            self.toggle_response_mode()
    ...
    def send_message(self):
        ...
        self.update_log(f"MODE: {self.current_mode.title()}", "INFO")
        ...
        self.worker = SearchWorker(text, self.memory, mode=self.current_mode)
    ...
    def set_ui_enabled(self, enabled: bool):
        ...
        self.mode_toggle_button.setEnabled(enabled)
        if enabled:
            self.status_label.setText("Ready")
            self.input_field.setFocus()
